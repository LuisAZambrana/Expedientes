<?php
    class db{
            ...
            try{
                ...
                if ($losdatos) {
                    ...
                    endforeach; }
                        ...
                        $boton_nuevo ='<a data-toggle="tooltip" data-placement="top" title="Nuevo Registro" ';
                        if ($this->FcGetHabiltarMenu($formularioid,3,$usuarioid,2) == 0){
                            $boton_nuevo.= ' aria-disabled="true" data-bs-toggle="modal" data-bs-target="#exampleModal" ';
                        }
                        else{   
                            $boton_nuevo.=' href="'.$ruta.'create.php"';
                        }
                         $boton_nuevo.= '><i class="bx bx-folder-plus"></i></a>';
                         $configuracion.= '
                         '.$boton_nuevo.'
                         ';

                         $configuracion.='<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                        <div class="modal-content">
                        <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel"><i class="bx bx-user-x"></i>    Permiso denegado</h1>
        ...
        public function configurar_grilla_personalizado_junto($grillaid,$condicion,$controles){
            try{
                $sql = "SELECT * FROM syma_grilla_h where grillaid=".$grillaid;
                $lagrilla = $this->fcGetSQL($sql,1,2);
                $sql_1 = "SELECT * FROM syma_grilla_i0 where visible = 1 and baja = 0 and grillaid =".$grillaid;
